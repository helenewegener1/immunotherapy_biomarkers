...
df_plot_TPMssGSEA <- readRDS('rds/04_01_TPMssGSEA_df_plot.rds') # Score column = TGEP
df_plot_DEAssGSEA <- readRDS('rds/04_02_DEAssGSEA_df_plot.rds') # Score column = TGEP
df_plot_ES <- readRDS('rds/04_03_ES_df_plot.rds') # Score column = TGEP
df_plot_signifinder <- readRDS('rds/04_04_signifinder_df_plot.rds') # Score column = TPM_TinflamSign
...
for (this_cancer in cancer_types){
  ...
  plots <- lapply(names(plot_list), function(name) {
    ...
    ggplot(
      df %>% filter(cancer == this_cancer),
      aes(y = TGEP, x = !!sym(var))
    ) +
      geom_violin() +
  ...
  wrap_plots(plots) 
  ggsave(glue('plot/04_11_TGEP_{this_cancer}_{var}.png'), width = 13, height = 9)
