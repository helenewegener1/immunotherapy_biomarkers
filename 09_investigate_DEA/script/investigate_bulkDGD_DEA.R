...
# 
# saveRDS(dea_files_clean, '09_investigate_DEA/out/DEA_files_clean.rds')
dea_files_clean <- readRDS('09_investigate_DEA/out/DEA_files_clean.rds')
...
for (this_cancer in cancer_types){
  for (var in meta_vars){
    ...
    ggsave(glue('09_investigate_DEA/plot/PCA_zscore_{this_cancer}_{var}.png'), height = 8, width = 12)
    
  }
}


############################ PCA of TGEP (Tinflam_Ayers) z scores ############################## 

geneset <- readRDS('05_genesets/out/geneset.rds')

TGEP_z_score_pca_prep <- dea_files_clean %>% 
  filter(genes %in% geneset$Tinflam_Ayers) %>% 
  dplyr::select(genes, sample, z_score) %>% 
  pivot_wider(names_from = genes, 
              values_from = z_score) %>% 
  column_to_rownames('sample') %>% 
...
for (this_cancer in cancer_types){
  for (var in meta_vars){
    ...
    ggsave(glue('09_investigate_DEA/plot/PCA_TGEP_zscore_{this_cancer}_{var}.png'), height = 8, width = 12)
