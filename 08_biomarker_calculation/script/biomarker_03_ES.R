...
for (this_study in studies){
  ...
  for (study_sample in study_samples){
    ...
    dea_file <- suppressMessages(
      read_csv(glue('04_DEA/out/DEA_{study_sample}.csv'), show_col_types = FALSE) %>% rename('genes' = '...1')
    )
...
# Export 
saveRDS(df_ES_meta, '08_biomarker_calculation/out/03_ES_df_plot.rds')
...
for (this_cancer in cancer_types){
  ...
  ggsave(glue('08_biomarker_calculation/plot/03_ES_cutoff_test_TGEP_{this_cancer}.png'), width = 13, height = 9)
