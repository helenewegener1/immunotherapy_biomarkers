...
for (this_cancer in cancer_types){
  ...
  for (this_geneset in names(geneset)){
    ...
    plot_list <- list(
      ...
      DEA_logFC_ssGSEA = df_plot_DEA_logFC_ssGSEA,
      DEA_zscore_ssGSEA = df_plot_DEA_zscore_ssGSEA,
      # ES        = df_plot_ES,
    ...
    plots$DEA_zscore_ssGSEA <- plots$DEA_zscore_ssGSEA + labs(
      title = glue('{this_cancer}: ssGSEA on approximated z-score from bulkDGD DEA'), 
      subtitle = 'z-score ~ sign(log2FC) * qnorm(1 - p_value/2)',
      y = glue('ssGSEA on {this_geneset} genes')
    )
    
    plots$DEA_logFC_ssGSEA <- plots$DEA_logFC_ssGSEA + labs(
      title = glue('{this_cancer}: ssGSEA on logFC from bulkDGD DEA'), 
      y = glue('ssGSEA on {this_geneset} genes')
    )
    ...
    for (this_geneset in names(geneset)){
      ...
      plots <- lapply(names(plot_list), function(name) {
        ...
        df %>% 
          filter(cancer == this_cancer) %>% 
          mutate(x_group = interaction(!!sym(var1), !!sym(var2), sep = "_")) %>% 
          ggplot(aes(y = !!sym(this_geneset), x = x_group)) +
            geom_violin() +
            geom_boxplot(width = 0.15, outlier.shape = NA, color = 'grey') + 
            geom_jitter(width = 0.05) +
            geom_text(
      ...
      # Export plots in one figure
      wrap_plots(plots) + plot_annotation(
        title = glue("{this_cancer} cancer, {this_geneset} genes split by {var1} and {var2}"),
        subtitle = glue("N genes of geneset: {geneset[[this_geneset]] %>% length()}")
        # caption = "Data source: mtcars"
      ) &
        theme(plot.title = element_text(face = "bold"))
